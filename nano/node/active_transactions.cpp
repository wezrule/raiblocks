...
void nano::active_transactions::insert_election_from_frontiers_confirmation (std::shared_ptr<nano::block> const & block_a, nano::account const & account_a, nano::uint128_t previous_balance_a, uint64_t & elections_count_a)
{
	auto insert_result = insert (block_a, previous_balance_a);
	if (insert_result.inserted)
	{
		insert_result.election->transition_active ();
		++elections_count_a;
		optimistic_frontier_elections.emplace (account_a, insert_result.election);
	}
}

bool nano::active_transactions::confirm_prioritized_frontiers (nano::transaction const & transaction_a)
{
	...
	auto max_active = node.config.active_elections_size / 200;
	auto should_confirm{ false };
	if (roots_size <= max_active && (check_time_exceeded || wallets_check_required || (!is_dev_network && low_active_elections && agressive_mode)))
	{
		...
		auto start_elections_for_prioritized_frontiers = [&transaction_a, &elections_count, max_elections, &lk, this](prioritize_num_uncemented & cementable_frontiers) {
			while (!cementable_frontiers.empty () && !this->stopped && elections_count < max_elections)
			{
				...
				if (!error)
				{
					if (!this->confirmation_height_processor.is_processing_block (info.head) && optimistic_frontier_elections.count (cementable_account.account) == 0)
					{
						...
						if (info.block_count > confirmation_height_info.height)
						{
							...
							this->insert_election_from_frontiers_confirmation (block, cementable_account.account, previous_balance, elections_count);
						}
					}
				}
				lk.lock ();
...
void nano::active_transactions::frontiers_confirmation (nano::unique_lock<std::mutex> & lock_a)
{
	...
	if (!disabled_confirmation_mode && bootstrap_weight_reached && !conf_height_capacity_reached && !all_cemented)
	{
		...
		auto time_spent_prioritizing_ledger_accounts = request_interval / (low_active ? 100 : 500);
		auto time_spent_prioritizing_wallet_accounts = request_interval / 250;
		auto time_spent_confirming_pessimistic_account = request_interval / (low_active ? 20 : 100);
		lock_a.unlock ();
		auto transaction = node.store.tx_begin_read ();
		...
		//auto should_confirm = confirm_prioritized_frontiers (transaction);
		//if (should_confirm)
		//{
			confirm_deepest_uncemented_blocks (transaction, node.network_params.network.is_dev_network () ? std::chrono::milliseconds (50) : time_spent_confirming_pessimistic_account);
		//}
		lock_a.lock ();
	}
}

void nano::active_transactions::confirm_deepest_uncemented_blocks (nano::transaction const & transaction_a, std::chrono::milliseconds account_traversal_a)
{
	auto i{ node.store.latest_begin (transaction_a, next_pessimistic_account) };
	...
	for (; i != n && !stopped && elections_count < max_elections; ++i)
	{
		...
		if (account_info.block_count > confirmation_height_info.height && optimistic_elections.get<tag_account> ().count (account) == 0)
		{
			...
			if (block && !node.confirmation_height_processor.is_processing_block (block->hash ()))
			{
				...
				// Check if the source is confirmed before trying to confirm the open
				...
				{
					// Source is confirmed, so we can confirm this block
					...
					insert_election_from_frontiers_confirmation (block, account, previous_balance, elections_count);
				}
			}
		}

		next_pessimistic_account = account.number () + 1;
...
void nano::active_transactions::prioritize_account_for_confirmation (nano::active_transactions::prioritize_num_uncemented & cementable_frontiers_a, size_t & cementable_frontiers_size_a, nano::account const & account_a, nano::account_info const & info_a, uint64_t confirmation_height)
{
	if (info_a.block_count > confirmation_height && !confirmation_height_processor.is_processing_block (info_a.head))
	{
		...
		if (optimistic_elections.get<tag_account> ().count (account_a) == 0)
		{
...
void nano::active_transactions::prioritize_frontiers_for_confirmation (nano::transaction const & transaction_a, std::chrono::milliseconds ledger_account_traversal_max_time_a, std::chrono::milliseconds wallet_account_traversal_max_time_a)
{
	...
	if (confirmation_height_processor.awaiting_processing_size () < confirmed_frontiers_max_pending_size)
	{
		...
		// Clean up old optimistic_elections
		for (auto i{ optimistic_elections.get<tag_election_started> ().begin () }; i != optimistic_elections.get<tag_election_started> ().end (); ++i)
		{
			if (i->election_started + 5min < std::chrono::steady_clock::now ())
			{
				i = optimistic_elections.get<tag_election_started> ().erase (i);
			}
		}

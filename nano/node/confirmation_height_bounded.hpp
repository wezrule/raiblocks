...
{
public:
	confirmation_height_bounded (nano::ledger &, nano::write_database_queue &, std::chrono::milliseconds, nano::logger_mt &, std::atomic<bool> &, nano::block_hash const &, std::function<void (std::vector<nano::confirmation_height::callback_data> const &)> const &, std::function<uint64_t()> const &);
...
private:
	...
	std::function<void (std::vector<confirmation_height::callback_data> const &)> notify_observers_callback;

...
void nano::confirmation_height_unbounded::process ()
{
	...
	do
	{
		...
		std::vector<confirmation_height::callback_data> block_callback_datas_required;
...
void nano::confirmation_height_unbounded::collect_unconfirmed_receive_and_sources_for_account (uint64_t block_height_a, uint64_t confirmation_height_a, nano::block_hash const & hash_a, nano::account const & account_a, nano::read_transaction const & transaction_a, std::vector<receive_source_pair> & receive_source_pairs_a, std::vector<confirmation_height::callback_data> & block_callback_data)
{
	...
	while ((num_to_confirm > 0) && !hash.is_zero () && !stopped)
	{
		...
		if (block)
		{
			...
			{
				if (!hit_receive && !block_callback_data.empty ())
				{
					...
					last_receive_details->send_callbacks_required.assign (block_callback_data.begin (), block_callback_data.end ());
				...
				receive_source_pairs_a.emplace_back (std::make_shared<conf_height_details> (account_a, hash, block_height, 1, std::vector<confirmation_height::callback_data>{ { block, sideband } }), source);
			}
			else if (is_original_block)
			{
				orig_block_callback_data.emplace_back (block, sideband);
			}
			else
			{
				if (!hit_receive)
				{
					...
					block_callback_data.emplace_back (block, sideband);
				}
				else
				{
					...
					last_receive_details->block_callback_data.emplace_back (block, sideband);
...
bool nano::confirmation_height_unbounded::cement_blocks ()
{
	...
	while (!pending_writes.empty ())
	{
		...
		if (pending.height > confirmation_height)
		{
			...
			notify_observers_callback (pending.block_callback_data);
			transaction.renew ();
		}
...
void nano::confirmation_height_unbounded::get_block_and_sideband (nano::block_hash const & hash_a, nano::transaction const & transaction_a, std::shared_ptr<nano::block> & block_a, nano::block_sideband & sideband_a)
{
	...
	else
	{
		...
		block_cache[hash_a] = { block_a, sideband_a };
		++block_cache_size;
	}
}
...
nano::confirmation_height_unbounded::conf_height_details::conf_height_details (nano::account const & account_a, nano::block_hash const & hash_a, uint64_t height_a, uint64_t num_blocks_confirmed_a, std::vector<confirmation_height::callback_data> const & block_callback_data_a) :
account (account_a),
hash (hash_a),
height (height_a),

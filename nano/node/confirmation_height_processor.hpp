...
public:
	...
	void add_cemented_observer (std::function<void(confirmation_height::callback_data)> const &);
...
private:
	...
	std::vector<std::function<void(nano::confirmation_height::callback_data)>> cemented_observers;
	std::vector<std::function<void()>> cemented_batch_finished_observers;

	nano::ledger & ledger;
	nano::write_database_queue & write_database_queue;
	confirmation_height_unbounded confirmation_height_unbounded;
	confirmation_height_bounded confirmation_height_bounded;
	std::thread thread;

	void set_next_hash ();
	void notify_observers (std::vector<nano::confirmation_height::callback_data> const & cemented_blocks);

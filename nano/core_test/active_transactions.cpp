...
namespace nano
{
TEST (active_transactions, confirm_deepest_uncemented_blocks)
{
	nano::system system;
	nano::node_flags flags;
	nano::node_config config (nano::get_available_port (), system.logging);
	...
	node.active.confirm_deepest_uncemented_blocks (node.store.tx_begin_read (), 500000ms); //50ms);
	{
		ASSERT_EQ (1, node.active.size ());
	...
	node.active.confirm_deepest_uncemented_blocks (node.store.tx_begin_read (), 50ms);
	...
	node.active.confirm_deepest_uncemented_blocks (node.store.tx_begin_read (), 1000ms);
}
}
